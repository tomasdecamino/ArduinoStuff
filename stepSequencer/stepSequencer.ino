...
int currentStep = 16;
boolean steps[] = {true, false, true, false, true, false, true, false};
int encoderPin1 = 2;
int encoderPin2 = 3;
int buttonPin = 4;
int state = 0;
...
void loop() {
  ...
  if (millis() - lastMillis > encoderValue) {
    colorStep((currentStep+1)*2);
    currentStep = (currentStep + 1) % 8;
    lastMillis = millis();
  }
  if(!digitalRead(9)){
    steps[7-(pos/10)]=!steps[7-(pos/10)];
    //state=0;
  }
  delay(10);
...
void colorStep(uint8_t s) {
  ...
  if (state==1) {
   // strip.setPixelColor(s, strip.Color(50, 200, 0));
    strip.setPixelColor((7-(pos/10))*2, strip.Color(200, 255, 255));
    if(steps[7-(pos/10)]) strip.setPixelColor((7-(pos/10))*2+1, strip.Color(255, 0,0));
    else strip.setPixelColor((7-(pos/10))*2+1, strip.Color(200, 255, 255));
  }
  else if(state==0) {
...
void updateEncoder() {
  ...
  if (state==1) {
    ...
    if (sum == 0b1110 || sum == 0b0111 || sum == 0b0001 || sum == 0b1000)  pos = (pos+1)%80;
  } else if(state==0){
